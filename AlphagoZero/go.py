...
class GameState(object):
    ...
    def get_legal_moves(self, include_eyes=True):
        ...
        self.__legal_move_cache = [PASS_MOVE]
    ...
    def get_score_of_empty(self):
        ...
        BLACK_score = 0
        WHITE_score = 0
        for empty, (br, wr) in reachability.iteritems():
            if (br, wr) == (True, False):
                BLACK_score += 1
    ...
    def do_move(self, action, color=None):
        ...
        if len(self.history) > 1:
            if len(self.history) >= 722:
                self.is_end_of_game = True
            elif self.history[-1] is PASS_MOVE and self.history[-2] is PASS_MOVE \
